...
import { StreamTranscriptItem } from "@/modules/interviews/types";
import { db } from "@/db";
...
const summarizer = createAgent({
  name: "summarizer",
  system: `
    You are an expert summarizer. You write readable, concise, simple content. You are given a transcript of a interview and you need to summarize it.
  ...
  async ({ event, step }) => {
    ...
    const transcriptWithSpeakers = await step.run("add-speakers", async () => {
      ...
      const agentSpeakers = await db
        .select()
        .from(interviewers)
        .where(inArray(interviewers.id, speakerIds))
        .then((interviewers) =>
    ...
    await step.run("trigger-smart-followup", async () => {
      await inngest.send({
        name: "interviews/smart-followup",
        data: {
...
export const interviewTimeoutEnd = inngest.createFunction(
  { id: "interviews/timeout-end" },
  { event: "interviews/end-call-after-timeout" },
  async ({ event, step }) => {
    const { interviewId } = event.data;

    await step.sleep("wait-for-timeout", "30s");
    
    const [existingMeeting] = await step.run("check-interview-status", async() => {
        return db.select().from(interviews).where(eq(interviews.id, interviewId));
    });
    ...
    if (existingMeeting && existingMeeting.status === "active") {
        ...
        console.log(`Call for interview ${interviewId} is not active. Skipping forced end.`);
    }
  },
);
...
export const smartFollowUp = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const [userData] = await step.run("fetch-user", async () => {
      return db.select().from(user).where(eq(user.id, interview.userId));
    });

    const { output } = await summarizer.run(`
You are Evalora’s Smart Follow-Up Interviewer.

Write a short, friendly **email message** (not a plain text paragraph) as a reminder for the user about their interview.

Format it properly like an email with:
- A short greeting
- 2–3 bullet points summarizing what was discussed or next steps
- A closing line (e.g., “Talk soon” or “Your AI interview assistant”)
...
`);
    ...
    await step.run("send-email", async () => {
      await resend.emails.send({
        from: "Evalora <noreply@resend.dev>",
        to: userData.email,
        subject: "Smart Follow-Up from Your Evalora Interview",
        html: `
          <div style="font-family: Inter, sans-serif; line-height: 1.6; color: #333; padding: 20px; max-width: 600px; margin: auto; border: 1px solid #eee; border-radius: 10px;">
            ...
            <p>Here’s a quick follow-up from your AI-powered interview yesterday:</p>
            <ul style="margin: 12px 0; padding-left: 20px;">
              ${listItems}
            </ul>
            <p style="margin-top: 16px;">Stay productive,<br><strong>— Evalora</strong></p>
          </div>
        `,
