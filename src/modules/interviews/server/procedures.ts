...
export const interviewsRouter = createTRPCRouter({
        ...
        const [createdMeeting] = await db
          ...
          await call.create({
            data: {
              ...
              custom: {
                interviewId: createdMeeting.id,
                interviewName: createdMeeting.name
              },
              settings_override: {
                transcription: {
                  language: "en",
          ...
          const [existingAgent] = await db
              .select()
              .from(interviewers)
              .where(eq(interviewers.id, createdMeeting.interviewerId));
    ...
    getOne: protectedProcedure
      ...
      .query( async ({ input, ctx }) => {
        const [existingMeeting] = await db
          ...
          .from(interviews)
          .innerJoin(interviewers, eq(interviews.interviewerId, interviewers.id))
          .where(
            and(
              eq(interviews.id, input.id),
              eq(interviews.userId, ctx.auth.user.id),
    ...
    .query( async ({ ctx, input }) => {
        const { search, page, pageSize, status, interviewerId } = input;
        ...
        const data = await db
          ...
          .where(
            and(
              ...
              interviewerId ? eq(interviews.interviewerId, interviewerId) : undefined,
            )
          )
